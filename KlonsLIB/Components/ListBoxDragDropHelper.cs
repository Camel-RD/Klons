...
namespace KlonsLIB.Components
{
    ...
    public class ListBoxDragDropHelper
    {
        ...
        public ListBoxDragDropHelper(ListBox draglistbox, ListBox droplistbox,
            DListBoxDragDropHelperEventListener ondrop)
            :this(draglistbox, droplistbox, ondrop, typeof(ListBox))
        {

        }

        public ListBoxDragDropHelper(ListBox draglistbox, ListBox droplistbox, 
            DListBoxDragDropHelperEventListener ondrop, Type boxtype)
        {
        ...
        public void MoveItem(ListBox lb, int fromnr, int tonr)
        {
            int k = lb.Items.Count;
            if (k < 1 || fromnr >= k || fromnr < 0 || tonr >= k || tonr < -1 || fromnr == tonr) return;
            var o1 = lb.Items[fromnr];
            bool b = false;
            if (tonr == -1)
            {
                lb.BeginUpdate();
                if (lb is CheckedListBox) b = (lb as CheckedListBox).GetItemChecked(fromnr);
                lb.Items.RemoveAt(fromnr);
                lb.Items.Add(o1);
                if (lb is CheckedListBox) (lb as CheckedListBox).SetItemChecked(lb.Items.Count - 1, b);
                lb.EndUpdate();
            ...
            else
            {
                ...
                if (lb is CheckedListBox) b = (lb as CheckedListBox).GetItemChecked(fromnr);
                lb.Items.RemoveAt(fromnr);
                lb.Items.Insert(tonr, o1);
                if (lb is CheckedListBox) (lb as CheckedListBox).SetItemChecked(tonr, b);
                lb.EndUpdate();
                lb.SelectedIndex = tonr;
            }
        }
        ...
        private void DropListBox_DragDrop(object sender, DragEventArgs e)
        {
            ...
            DoScrollA(0);
            if (e.Data.GetDataPresent(BoxType))
            {

                ListBox lbox = (ListBox)e.Data.GetData(BoxType);
        ...
        void DoScrollA(int scrollDelta)
        {
            lock (this)
            {
                if (scrollDelta == 0)
